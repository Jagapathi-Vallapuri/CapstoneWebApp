    ...
    <Card>
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          <div className="h-12 w-12 flex-none rounded-full bg-indigo-600 text-white grid place-items-center font-semibold">{(me?.name || '?').slice(0,1).toUpperCase()}</div>
          <div>
            <div className="text-lg font-semibold text-gray-900">{me?.name || me?.email || 'User'}</div>
        ...
        <div className="flex items-center gap-2">
          <button onClick={onEdit} aria-label="Edit medical profile" className="inline-flex items-center gap-2 rounded-md px-3 py-1 text-sm bg-white border border-gray-200 hover:bg-gray-50">
            <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 text-gray-700" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15.232 5.232l3.536 3.536M9 11l6 6L21 11l-6-6-6 6z" /></svg>
            <span className="text-sm text-gray-700">Edit</span>
          </button>
          {/* back-to-upload removed: use top-bar Upload button */}
        </div>
      </div>
        ...
        <div>
          ...
          <div className="mt-2 grid grid-cols-1 sm:grid-cols-2 gap-2">
            {summaryFields.slice(0, expanded ? summaryFields.length : 4).map(f => (
              <div key={f.k} className="rounded-md border border-gray-100 bg-gray-50 p-2 text-sm">
